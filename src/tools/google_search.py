...
class GoogleSearch(ToolBase):
    ...
    def call_search(self, query: str, api_key: str, engine: str = "google", location: str = "") -> dict[str, any] | tuple[int, str]:
        """
    ...
    def search(self, search_query: str) -> str:
        ...
        """
        # Load the API key
        api_key = self.load_api_key(CREDENTIALS_PATH)


        # Perform the search
        results = self.call_search(search_query, api_key, location="")

        # Check if the search was successful
        if isinstance(results, dict):
            # Format and return the top search results as JSON with updated key names
            top_results = self.format_top_search_results(results)
...
if __name__ == "__main__":
    search = GoogleSearch()
    q = "Best gyros in Barcelona, Spain"
    result_json = search.search(q)
    print(result_json)